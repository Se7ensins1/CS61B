...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private void swim(int index) {
        ...
        if (parentIndex < 1 || contents[index].myPriority >= contents[parentIndex].myPriority) {
            return;
        }
    ...
    private void sink(int index) {
        ...
        if (child > size || contents[index].myPriority <= contents[child].myPriority) {
            return;
        }
    ...
    @Override
    public T removeMin() {
        Node initialRoot = contents[1];
        swap(1, size);
        contents[size] = null;
        size -= 1;
        sink(1);
        return initialRoot.myItem;
    }
    ...
    public void changePriority(T item, double priority) {
        for (int i = 1; i < size; i++) {
            if (contents[i].myItem.equals(item)) {
                boolean increasePriority = contents[i].myPriority > priority;
